...
describe('vector sources', () => {
  it('records where the rows came from', () => {
    // Informational: each block above names its copy in its title.
    expect([MANAGER_ROWS.source, METHOD_ROWS.source].every((source) => source.length > 0)).toBe(true);
  });
});
